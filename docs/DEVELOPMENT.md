...
- 核对用户提供的 nemc-form-script UI 继承链，它仍使用原版 common.text_edit_box；其 30 UI 单位与本编辑器缩放后的 30 设计单位不同。对照 0.5 / 0.625 / 0.75 倍后，模板采用 0.5 倍，保持原字体，保留完整文字裁剪范围，不新增字体或图片。
- 五种尺寸、真实中文输入法、删除、长文本及重开共 87 项通过，1080p 与 720p 实际截图均已查看。验证脚本改为读取当前模板字号后恢复。
