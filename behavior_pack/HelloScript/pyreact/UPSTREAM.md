...
同日白名单复核撤回了 `host.py` / `native.py` 对引擎内部 `gui` 模块的输入路由合并补丁，该模块不在官方允许列表中。框架现在直接使用公开 SDK，不拦截引擎输入路由。应用层模型排序通过 `PendingLayers` 合并重复请求，每帧最多执行 4 次公开 `SetLayer`，触屏按住期间延后排序，松手后完成；卸载控件的过期请求会被清除。
