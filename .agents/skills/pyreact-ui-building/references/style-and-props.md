...
游戏行为包只能导入官方 ModSDK 白名单模块和项目自有模块。曾用于合并输入路由的 `native.batch_input_routes()` 已移除：它依赖不在白名单中的 `gui`，不可恢复此做法。三维分块使用固定层级和共享着色器深度，旋转期间不修改模型层级；不能拦截引擎内部输入路由。空 Panel/Label 及其子类不依赖刷新后的原生尺寸；绝对定位的两侧锚点也可确定相应轴尺寸，无需为此再次排版整棵树。
