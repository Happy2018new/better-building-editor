...
from verify_world_tools import game, equip, snapshot, call, input_step
...
_result=item.GetPlayerItem(api.GetMinecraftEnum().ItemPosType.CARRIED,0)''',True)
    ...
    try:
        ...
        for name in ('terminal','survey_wand'):
            ...
            time.sleep(2.)
            report[name+'_first_later']=snapshot('astral_'+name+'_first_later')
        ...
        time.sleep(2.)
        report['third_later']=snapshot('astral_survey_third_later')
        ...
        recording=subprocess.Popen(['ffmpeg','-y','-hide_banner','-loglevel','error',
            ...
            '-t','4','-vf','scale=960:-2','-c:v','libx264','-preset','veryfast',
            '-crf','18',str(OUT/'astral_staff_motion.mp4')],stderr=subprocess.PIPE)
        try:
            time.sleep(.4)
            input_step('/run',steps=[{'do':'key','keys':'w','hold_ms':450},
                                     {'do':'key','keys':'s','hold_ms':450}])
            unused,errors=recording.communicate(timeout=12)
            assert recording.returncode==0,errors.decode('utf8','replace')
        finally:
            if recording.poll() is None:
                recording.terminate()
                recording.communicate(timeout=5)
        report['motion_video']=str(OUT/'astral_staff_motion.mp4')
