...
def main():
    ...
    time.sleep(1)
    ui.check('largest volume uses exact native dimensions',state['sceneSize']==[64,100,64] and state['sceneScale']==1)
    ui.check('one coherent native preview pair',len(ui.nodes('PaperDoll')) == 2)
    snapshot('exact_solid_verified')
    diagnostic({'fixture':'landmarks'});wait_preview()
    ui.click('俯视'); time.sleep(.6)
    click_point((63.5,100.,63.5))
    ui.check('overview picking reaches the far high column',diagnostic()['focused']==[63,99,63])
    ...
    ui.call('set_input',ui.nodes('Input',center)[0]['id'],'63, 99, 63');wait_preview()
    state=diagnostic()
    ui.check('precise crop remains located at actual far coordinates',state['origin']==[32,68,32] and state['sceneSize']==[32,32,32])
    click_point((63.5,100.,63.5))
    ui.check('precise picking and overview picking agree',diagnostic()['focused']==[63,99,63])
