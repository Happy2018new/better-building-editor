...
def main():
    ...
    diagnostic({'fixture':'landmarks'});wait_preview();ui.click('整栋总览');wait_preview()
    ...
    diagnostic({'chunk':[63,127,63]});wait_preview()
    state=diagnostic()
    ui.check('current chunk retains actual far coordinates',state['origin']==[48,112,48] and state['sceneSize']==[16,16,16])
    click_point((63.5,128.,63.5))
