...
def screen_point(pos):
    ...
    cam.pivot=state.get('cameraPivot')
...
def hover(pos):
    ...
    assert capture.user32.GetForegroundWindow()==window['hwnd'], 'game lost foreground during hover check'
    left,top,width,height=capture._window_rect(window['hwnd']);scale=width/root['width']
    target=(int(left+(box['x']+x)*scale),int(top+(box['y']+y)*scale))
    capture.user32.SetCursorPos(*target);time.sleep(.3)
    actual=capture.POINT();capture.user32.GetCursorPos(ctypes.byref(actual))
    assert (actual.x,actual.y)==target, ('cursor moved during hover check',target,(actual.x,actual.y))
...
def main():
    ...
    time.sleep(.3)
    ui.click('工作台');diagnostic({'fixture':'solid','size':[24,16,24]});wait_preview();ui.click('俯视')
    for mode in ('浏览','选取','换材质','擦除','吸管','框选'):
        ...
        if a['cursorCell']!=[6,15,8] or b['cursorCell']!=[10,15,8]:
            print('hover trace',mode,a,b,flush=True)
    ...
    ui.check('touch erase deletes exactly once on release',after['blocks']==before-1)
    touch((10.5,16,8.5),True)
    ui.check('touch orbit does not erase',diagnostic()['blocks']==before-1)
    ui.click('换材质');diagnostic({'camera':[0,90,1]});time.sleep(.7)
    touch((10.5,16,8.5))
    ui.check('touch paint directly selects the touched cell without confirmation',diagnostic()['focused']==[10,15,8] and '确认换材质' not in ui.labels())
    ui.click('选取')
    touch((8.5,16,8.5))
